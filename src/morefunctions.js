 ...
 * 
 * MoreFunctions offical release!
 * Release 1.x.x Amur Tiger 25/09/2017
 * 
 */


// IF SET TO TRUE: THE smallStandardMF styles will be added
var autoload = false;
...
var included = [
	...
	"write",
	"writeTo",
	"autoLoad", // Only for advanced users! 
 ...
 * Popup (, alert), Html or text (, plain)
 ...
 *     --> text
 * 
 * i = 1 to prevent the undifined bug
 * @returns
...
/**
 * 
 * @returns time (Hours:Minutes:Seconds)
 */
function startClock() {
	function startTime() {
	    var today = new Date();
...
/**
 * 
 * @returns if user has java enabled (NOTICE: there's a difference between java and javaSCRIPT)
 */
function userJavaOn() {
 ...
 *
 *
 * Added in: 1.0.1 (Release + Beta: Amur Tiger (Beta) )
 * @param doSet
 * --> Does the conten of @param id has to be changed to @param text?
 * ---> Yes?
 * ----> Don't set or set to true
 * ---> No?
 * ----> Set to true
 */

function writeTo(text, id, doSet) {
	...
	if (!isset(text)) {
        ...
        if (doSet == false) {
            var pre = document.getElementById(id).value;
            document.getElementById(id).innerHTML += text;
            // Has @code text really been added to @code id?
            // Yes?
            if (pre + text == document.getElementById(id).value) {
                return true;
            } else {
                while (pre + text == document.getElementById(id).value && x > 3) {
                    // retry 3 times
                    $x++;
                    document.getElementById(id).innerHTML += text;
                };
                // Is @code id really @code text?
                if (pre + text == document.getElementById(id).value) {
                    // Yes?
                    return true;
                } else {
                    // No? (error 25)
                    return false;
                    throw 'MF: err 25: Something went wrong, while adding to "' + id + '": "' + text + '"';
                };
                
            };
        } else if (!isset('doSet') || doSet == true) {
            document.getElementById(id).innerHTML = text;
		...
		if (document.getElementById(id).value == text) {
			...
			return true
                ...
                if (document.getElementById(id).value == text) {
                    ...
                    // No? (error 26)
                    return false;
                    throw 'MF: err 26: Something went wrong, while changing "' + id + '"\'s content to "' + text + '"';
                };
            };
        };
	};
 ...
 * @returns
 * --> Only an error when no permissions are detected
 */
...
if (hasAlert == true) {
	window.alert = function(msg){
		...
		console.log('MF: NOTICE.: 6: No permission, please first run addMFalert()');
};
/**
 * 
 * @param fahrenheit
...
function toCelsius(fahrenheit) {
	if (!isset(fahrenheit)) {
		throw 'MF: err. 28: fahrenheit was not set!';
		return false;
	} else {
...
function toFahrenheit(celcius) {
	if (!isset(celcius)) {
		throw 'MF: err. 29: celcius was not set!';
	} else {
...
/**
 * Just the smaller / part of the MF styles
 * @returns
 * --> True on success
 */
function getSmallMFstyles() {
	var x = document.createElement("DIV");
	var t = document.createTextNode('<link href="https://fonts.googleapis.com/css?family=Asap+Condensed|Encode+Sans+Expanded|Lato|Open+Sans|PT+Sans+Caption|Roboto|Roboto+Condensed|Ubuntu|Ubuntu+Condensed|Ubuntu+Mono" rel="stylesheet">');
	var tt = document.createTextNode("<!--      @import url('https://fonts.googleapis.com/css?family=Asap+Condensed|Encode+Sans+Expanded|Lato|Open+Sans|PT+Sans+Caption|Roboto|Roboto+Condensed|Ubuntu|Ubuntu+Condensed|Ubuntu+Mono');       font-family: 'Roboto', sans-serif;     font-family: 'Encode Sans Expanded', sans-serif;     font-family: 'Open Sans', sans-serif;     font-family: 'Asap Condensed', sans-serif;     font-family: 'Lato', sans-serif;     font-family: 'Roboto Condensed', sans-serif;     font-family: 'PT Sans Caption', sans-serif;     font-family: 'Ubuntu Mono', monospace;     font-family: 'Ubuntu Condensed', sans-serif;     font-family: 'Ubuntu', sans-serif;       -->");
	var newBodyStyle = document.createTextNode("<style>body { font-family: 'Ubuntu', sans-serif; }</style>");
